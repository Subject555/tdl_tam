...
public class Machine {
	...
	public final static int LOADop = 0, LOADAop = 1, LOADIop = 2, LOADLop = 3,
			...
			SUBRop = 9, PUSHop = 10, POPop = 11, JUMPop = 12, JUMPIop = 13,
	...
	public String getLabel(int d) {
		String l = labels.get(new Integer(d));
		if (l == null)
			l = ""; //$NON-NLS-1$
		return l;
	}
	...
	static String reg_name[] = { "CB", "CT", "PB", "PT", "SB", "ST", "HB",
			"HT", "LB", "L1", "L2", "L3", "L4", "L5", "L6", "CP" };

	static String inst_name[] = { "LOAD", "LOADA", "LOADI", "LOADL", "STORE",
			"STOREI", "CALL", "CALLI", "RETURN", "SUBR", "PUSH", "POP", "JUMP",
			"JUMPI", "JUMPIF", "HALT" };

	static String fun_name[] = { "BNeg", "BOr", "BAnd", "BOut", "BIn", "B2c",
			"B2i", "B2s", "COut", "CIn", "C2b", "C2i", "C2s", "INeg", "IAdd",
			"ISub", "IMul", "IDiv", "IMod", "IEq", "INeq", "ILss", "ILeq",
			"IGtr", "IGeq", "IOut", "IIn", "I2b", "I2c", "I2s", "MVoid",
			"MAlloc", "MFree", "MCompare", "MCopy", "SAlloc", "SCopy",
	...
	public final static int booleanSize = 1, characterSize = 1,
			...
			linkDataSize = 3 * addressSize, falseRep = 0, trueRep = 1,
			maxintRep = 16777216;
	...
	/* Fonctions Entiers */
	INeg = 13, IAdd = 14, ISub = 15, IMul = 16, IDiv = 17, IMod = 18, IEq = 19,
			INeq = 20, ILss = 21, ILeq = 22, IGtr = 23, IGeq = 24, IOut = 25,
			IIn = 26, I2B = 27, I2C = 28, I2S = 29,
			/* Fonctions Memoires */
			MVoid = 30, MAlloc = 31, MFree = 32, MCompare = 33, MCopy = 34,
			/* Fonctions Chaines */
			SAlloc = 35, SCopy = 36, SConcat = 37, SOut = 38, SIn = 39,
			S2B = 40, S2C = 41, S2I = 42;
	...
	final static int SB = 0, HB = MEMSIZE - 1;
	...
	private static String sstatus[] = {
			Messages.getString("Machine.Program_Running"), //$NON-NLS-1$
	...
	private int readInt() throws IOException {
		...
		do {
			...
			// System.err.print("cc1 = " + currentChar );
		...
		if (Character.isDigit((char) currentChar))
			do {
				...
				currentChar = System.in.read();
				// System.err.print("cc3 = " +currentChar );
	...
	private void readStrings(int n, DataInputStream f) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			...
			// f.read(s);
			// addString(new String(s));
			// System.err.println(new String(s) + "#" + i);
			int ls = f.readInt();
			String s = f.readUTF();
			addString(s);
	...
	private void readLabels(int n, DataInputStream f) throws IOException {
		...
		for (int i = 0; i < n; i++) {
			...
			// System.err.println(new String(s) + "#" + sn);
		}
	}
	...
	public Instruction read(DataInputStream input) throws IOException {
		...
		try {
			...
			inst.d = i2;
			// System.err.println(i1 + " " + i2 + " op=" + inst.op + " r="
			// + inst.r + " n=" + inst.n + " d=" + inst.d);
	...
	public void mnext() throws TamException {
		...
		// System.err.println(sstatus [status]);
		code[CP].s = true;
	}
	...
	public void loadObjectProgram(ByteArrayOutputStream out)
			throws TamException {
		try {
			if (out == null)
				throw new TamException(Messages.getString("Machine.Null_Error")); //$NON-NLS-1$
			DataInputStream objectStream = new DataInputStream(
			...
			// System.err.println("nl = " + nl + "ns = " + ns + "ni = " + ni);
			int addr = CB;
			code = new Instruction[ni];
			for (int i = 0; i < ni; i++) {
				code[addr++] = read(objectStream);
				// System.err.println(writeInst(code[i]));
			}
			objectStream.close();
			CT = addr;
			mreset();
			...
			throw new TamException(
					Messages.getString("Machine.Opening_Error") + s); //$NON-NLS-1$
		} catch (IOException s) {
			CT = CB;
